...
public class DumpLoadEndianIntTest extends TestCase {
	...
	public void doTest( boolean littleEndian, int numberFormat, boolean stripDebug, 
        ...
        try {
            ...
            Prototype p = state.compilePrototype(reader, "script");
            
            // double check script result before dumping
            LuaFunction f = new LuaClosure(p, state.globals, state);
            LuaValue r = f.call();
            ...
            f = state.load(is, "dumped", "b", state.globals).checkfunction();
            r = f.call();
            actual = r.tojstring();
